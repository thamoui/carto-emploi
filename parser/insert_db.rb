...
@urls_unclean= []
conn.exec( "SELECT * FROM parse WHERE NOT EXISTS (SELECT offer_id FROM job_offers WHERE (parse.id = job_offers.offer_id));" ).map do |result|
...
0.step(@urls_id.length-2, 2) do |num|
  ...
  puts "//////////// hey ! Il n'y a aucune nouvelle offre à parser /////////////"
end
conn.close
